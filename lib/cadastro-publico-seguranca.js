...
export function obterSegredoRateLimit(env = process.env) {
  const segredoExclusivo = env.BARBERVISION_RATE_LIMIT_SECRET?.trim();
  if (segredoExclusivo?.length >= 32) return segredoExclusivo;

  const segredoBase = env.TURNSTILE_SECRET_KEY?.trim();
  if (segredoBase) {
    return createHmac("sha256", segredoBase)
      .update("barbervision:rate-limit:v1", "utf8")
      .digest("hex");
  }

  if (env.NODE_ENV === "development") {
    return "barbervision-development-rate-limit-secret";
  }

  throw new Error("Configure BARBERVISION_RATE_LIMIT_SECRET ou TURNSTILE_SECRET_KEY.");
}
