...
- as doze migrations passaram por reset e pgTAP 205/205; houve ensaio histórico 8–4, incremental 10–9 e da migration 11, seguido de roll-forward; a migration 12 é limpeza operacional exata;
...
Os gates locais de banco, concorrência, integração e E2E já possuem evidência. Em 26/08/2026, o banco remoto foi reconstruído sobre as doze migrations, o Render permaneceu Live, o lifecycle do funcionário foi revalidado e o Worker/Cron Cloudflare entrou em operação. A sequência vigente é: provar retry/alerta remoto; fechar templates e casos adversários restantes de Auth; executar a matriz remota; fechar reatribuição, transferência de dono e seleção multi-tenant; então implementar privacidade. Consulte [Estado de validação](ESTADO-VALIDACAO.md).
